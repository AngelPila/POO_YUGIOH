...
def mostrar_tableros(tablero_jugador, tablero_bot):
    print(f"\n{"-"*76}TABLERO{"-"*76}")
    print(f"\n{"-"*73}Tablero del Bot{"-"*73}\n")
    print(f"{" "*75}-Monstruos-{" "*75}") 
    linea = ""  # Inicializa una cadena vacía
    i = 1
    for carta in tablero_bot.esp_monster:
        
        if carta.area.value == CartaUbicacion.BDOWN:
            linea += "La carta no es visible, "  # Agrega el mensaje a la cadena
        else:
            if not isinstance(carta, Espacio):
                linea += f"|[{i}]. {carta.nombre}, {carta.ataque}, {carta.defensa}|\t"
                i += 1
            else:
                linea += f"| [{i}]. Vacio |\t"
                i += 1
    ...
    print(f"\n")    
    # Magia/Trampa del Bot
    print(f"\n{' '*74}-Magia/Trampa-{' '*74}")
    linea = ""
    i = 1
    for carta in tablero_bot.esp_magtrp:
        if carta.area.value == CartaUbicacion.BDOWN:
            linea += "| La carta no es visible | "
        else:
            if not isinstance(carta, Espacio):
                linea += f"| [{i}]. {carta.nombre} |\t "
            else:
                linea += f"| [{i}]. Vacio |\t "
        i += 1
    print(linea.rstrip(", "))
    ...
    # Tablero del Jugador
    print(f"\n{'-'*73}Tablero del Jugador{'-'*73}\n")

    # Monstruos del Jugador
    print(f"{' '*75}-Monstruos-{' '*75}") 
    linea = ""
    i = 1
    ...
    while not jugador.jugadorDerrotado() and not bot.botDerrotado():
        ...
        if turno_jugador:
            ...
            while ataque == 0:
                ...
                if accion == "1":
                    indice_idx = input("Elige el índice de la carta a invocar: ")
                    while not indice_idx.isdigit():
                        indice_idx = input("Elige un indice valido: ")
                    idx = int(indice_idx) - 1
                    carta = jugador.cartas_mano[idx]
                    if isinstance(carta, CartaMonstruo):
                        posicion = int(input("Elige posición (1-3): "))
                        while posicion not in [1,2,3]:
                            posicion = int(input("Elige posición (1-3): "))
                    ...
                    indice_idx = input("Elige el índice de la carta mágica a usar: ")
                    while not indice_idx.isdigit():
                        indice_idx = input("Elige un indice valido: ")
                    idx = int(indice_idx) - 1
                    carta = jugador.cartas_mano[idx]
                    ...
                    # Atacar
                    posicion = input("Elige el espacio desde donde atacar (1-3): ")
                    while posicion not in [1,2,3]:
                        posicion = input("Elige el espacio desde donde atacar (1-3): ")
                    carta_atacante = tablero_jugador.esp_monster[int(posicion) - 1]
